...
final class CvViewModel
{
    public function make(?string $profileSlug = null, bool $authorized = false): array
    {
        ...
        return [
            ...
            'profile' => $profile ? ['slug' => $profile->slug(), 'title' => (string) $profileValue('title'), 'recipient' => (string) $profileValue('recipient_name', ''), 'year' => (string) $profileValue('application_year', ''), 'context' => (string) $profileValue('optional_context', '')] : null,
    ...
    private function plainText(mixed $bard): string { if (is_string($bard)) return trim(strip_tags($bard)); if (! is_array($bard)) return ''; $walk = function (array $nodes) use (&$walk): array { $parts = []; foreach ($nodes as $node) { if (isset($node['text'])) $parts[] = $node['text']; if (isset($node['content']) && is_array($node['content'])) $parts = [...$parts, ...$walk($node['content'])]; } return $parts; }; return trim(implode(' ', $walk($bard))); }
