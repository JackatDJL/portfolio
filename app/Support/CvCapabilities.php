...
final class CvCapabilities
{
    ...
    {
        return hash_equals($stored, $path);
    }

    public static function sessionAllows(Request $request, string $path): bool
    {
        ...
        if (! $record || ($record->expires_at && now()->isAfter($record->expires_at))) {
            return false;
        }
